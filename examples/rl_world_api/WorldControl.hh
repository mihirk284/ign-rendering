...
class WorldControl
{
    private: std::string engineName;
    private: std::vector<std::string> engineNames;
    private: RenderEngine *engine;
    private: ScenePtr scene;
    std::vector<common::ConnectionPtr> connPtrs;
    private: std::vector<CameraPtr> cameras;
    private: std::vector<GpuRaysPtr> gpuRays;
    private: std::vector<DirectionalLightPtr> dirLights;
    // private: std::vector<LightPtr> lights;
    private: std::vector<VisualPtr> visuals;
    public: ScenePtr GetScene();
    public: bool DefaultInitWorld();
    public: bool PopulateWorld();
    public: CameraPtr SpawnCamera(std::string _sensorName);
    public: GpuRaysPtr SpawnGpuRays(std::string _sensorName);
    public: std::vector<float *> scans;
    public: void SetWorldPose(SensorPtr _ptr, math::Pose3d _pose);
    public: void CaptureReading(SensorPtr _ptr);
}

void WorldControl::WorldControl()
{
    this->engineName = "ogre2";
    this->engineNames.push_back(this->engine);
    try
    {
        this->engine = rendering::engine(this->engineNames[0]);
    }
    ...
    ignmsg << "Successfully created the engine & scene" << std::endl;
}

ScenePtr WorldControl::GetScene()
{
    return this->scene;
}

CameraPtr WorldControl::SpawnCamera(std::string _sensorName)
{
    return this->scene->CreateCamera(_sensorName);
}

GpuRaysPtr WorldControl::SpawnGpuRays(std::string _sensorName)
{
    GpuRaysPtr _tempSensorPtr = this->scene->CreateGpuRays(_sensorName);
    int hRayCount = _tempSensorPtr->HorizontalRayCount();
    int vRayCount = _tempSensorPtr->VerticalRayCount();
    int channels = _tempSensorPtr->Channels();
    scans.push_back(new float[hRayCount * vRayCount * channels]);
    this->connPtrs.push_back(_tempSensorPtr->ConnectNewGpuRaysFrame(
        std::bind(&::OnNewGpuRaysFrame, scans[connPtrs.size()-1],
          std::placeholders::_1, std::placeholders::_2, std::placeholders::_3,
          std::placeholders::_4, std::placeholders::_5)));
    return _tempPtr;
}

bool WorldControl::PopulateWorld()
{
    // set based on config files? 
    // would be super cool to make dynamics worlds based on config files
}

bool WorldControl::DefaultInitWorld()
{
    this->cameras.push_back(this->SpawnCamera("camera"));
    this->gpuRays.push_back(this->SpawnGpuRays("lidar1"));
}

void WorldControl::SetWorldPose(SensorPtr _sensorPtr, math::Pose3d _pose)
{
  _sensorPtr->SetWorldPose(_pose);
  return;
}

void WorldControl::CaptureReading(SensorPtr _sensorPtr)
{
  _sensorPtr->Update();
}
